 ...
 */
function readSkillData(skills: Skill[], i: number, dbSkill: DBSkill) {
  ...
  skills[i + 1].damage.critical =
    parseInt(dbSkill.easyrpg_critical_hit_chance) > 0;
...
export async function updateSkills(
  ...
  try {
    ...
    database.skills[0].Skill.forEach(
      (Skill: DBSkill, /** @type {number} */ i: number) => {
